...
// @Success 200 {object} data.UserAddressVO
...
func AddUserAddress(c *gin.Context) {
	...
	userAddress, err := service.GetUserAddressService().CreateUserAddress(c, userAddress)
	if err != nil {
...
func UpdateUserAddress(c *gin.Context) {
	...
	userAddress.UserID = userId.(int)
	err := service.GetUserAddressService().UpdateUserAddress(c, userAddress)
	if err != nil {
		c.JSON(http.StatusInternalServerError, data.BaseResponse{Code: http.StatusInternalServerError, ErrMsg: err.Error()})
		return
	}
...
// @Accept json
// @Produce json
// @Success 200 {array} data.UserAddressVO
// @Failure 500 {object} data.BaseResponse
...
func ListUserAddresses(c *gin.Context) {
	...
	userAddresses, err := service.GetUserAddressService().GetUserAddresses(c, userId.(int))
	if err != nil {
		c.JSON(http.StatusInternalServerError, data.BaseResponse{Code: http.StatusInternalServerError, ErrMsg: err.Error()})
		return
	}
...
func DeleteUserAddress(c *gin.Context) {
	...
	err = service.GetUserAddressService().DeleteUserAddress(c, id, userId.(int))
	if err != nil {
		c.JSON(http.StatusInternalServerError, data.BaseResponse{Code: http.StatusInternalServerError, ErrMsg: err.Error()})
		return
	}
